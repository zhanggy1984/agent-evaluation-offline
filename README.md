...
**闭环分工**
- **上游（online 平台）**：采集 agent 的结构化错误事件 → 按 `input_hash` 归一聚类（L1/L2 分层）→ 组装为回流信封 → 写入回流收件箱；
- **本仓**：拉取信封 → 依 `evidence.input` **复现该错误场景**并构造回归用例 → 判定器给出 `passed / failed` → **结果回推**，触发该 agent 的 error 回归 run；
- **收口**：运营对簇发起认领（指定修复版本）后，`rejudge` 后台作业自动重放已落库结果，收敛该簇为 `fixed`——**认领后无需重新推送**。
...
不依赖线上真实事故，而是**主动注入确定性故障**构造 error——例如将依赖域名指向黑洞地址制造 DNS / 连接类失败、
在依赖侧制造超时——再观察该 error 能否自动走完全部环节。这种方式**可重复、可控、可对照**，
且每一步都有落库产物可核（ES 事件 → 聚类行 → 回流信封 → 回归用例 → 判定结果 → 收口记录）。
