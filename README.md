...
- **报告导出**：run 明细一键导出 PDF / Excel，审计留档；token 明文仅返回一次，DB 存 sha256。
...
- **告警通知**：run 失败 / 分数漂移 / 过拟合自动通知（企微 / 邮件），恢复去重。
...
- **LLM-judge**：语义维度（factuality/reasoning）用六级锚点 rubric（1.0/0.8/0.6/0.4/0.2/0.0）+ 理由 + 置信度 confidence，不输出模糊连续分；
- **分数合成**：加权平均 + 每维度阈值门禁（pass/fail），N/A 剔除重归一化，error 率硬门禁。

### 3. 金标准 = 校准集，驱动基线对比与漂移检测
`is_gold` 用例既是绝对水平基线（baseline_target 从金标准校准集自动标定），也是 judge 校准集——元评测定期检测 judge 一致率漂移、告警通知（run 失败 / 分数漂移 / 过拟合），金标准误标会导致首测告警（已踩过坑，见「常见问题」）。
...
### 6. 人工复核回写（低置信度闭环）
judge 置信度低于阈值 → 任务转 `pending_human`，run 阻塞在 scoring；Staff（admin/evaluator）可 approve 采纳原判定 / reject 改判或放弃，复核后自动触发 `score_run` 收敛。留出集（held-out）对 owner 隐藏，保证标注独立性。
...
- **报告安全**：token 明文仅返一次、DB 存 sha256、Excel 页脚水印；
...
- 骨架脚手架自动生成场景清单 + 用例骨架（draft），黄金答案永远人工标注。
...
**关键链路**：触发评测（手动）→ 契约探测（L0 硬拦截）→ 逐 case：reset(seed) 造数归位 → 调用 agent 接口（采集 answer/usage/timing/tool_call）→ 断言 + judge 评分 → scorer 汇总（加权平均 + 维度门禁 + run 终态）→ 门禁墙呈现 + 未达标摘要 → 低置信度人工复核回写 → PDF/Excel 报告导出。
...
| 前端 | Vue3 + Vite + Element Plus + Pinia + ECharts | 12 个业务页面，JS 非 TS |
| 数据库 | MySQL 8 | utf8mb4，业务权威数据源 |
| LLM judge | DeepSeek（openai 兼容） | 语义维度六级 rubric 精评 + 置信度 |
...
│   │   ├── judge/           # LLM-judge 六级 rubric + 人工复核 pending_human
...
│   ├── src/
│   │   ├── views/           # Dashboard/Performance/Cost/Coverage/Issues/MetaEval/Review/...
│   │   ├── api/             # axios 接口模块
...
**实测全绿（2026-08）：后端单测 511 + 集成测试 25 + 前端单测 20 = 556 项。**

后端单测覆盖（`backend/tests/`，511 项）：

- **runner 链路**：orchestrator（重试 / 限流）、executor、scorer、run 超时、契约探测 probe、engine、assembler；
...
- **平台能力**：仪表盘聚合、基线达标分、骨架生成、标注、告警通知、邮件、审计、安全（响应头/SSRF）、熔断器、幂等清理。

集成测试（`tests/integration/`，25 项，`RUN_INTEGRATION=1` 才连库）：完整 run 执行、scanner 回收、7.6 多 worker 互斥、漂移检测。
...
# （详见 tests/integration/conftest.py；转发容器已存在则复用）：
...
# 若宿主经 socat 连库被 MySQL 拒绝来源（1045 Access denied @172.19.0.1），
# 改在 backend 容器内临时装 pytest 跑（连 mysql:3306，来源为容器 IP 不受拒）：
...
| 金标准首次评测即告警（分数漂移） | 检查 gold 用例是否误标了 judge 实判 0 的维度（只标 factuality 等实测维度） |
