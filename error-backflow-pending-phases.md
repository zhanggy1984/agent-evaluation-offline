...
|---|---|---|
| **P4-1** | `T-3.17` 四仓镜像烤码早于本批 + `T-3.18`/cs 各 2 条真机未复验 | 卡在「镜像早于本批」；`task.md:299/:300` **台账自陈未验收**（**⚠️ 2026-09-16 订正：镜像面已复验** —— 批 6a 于 2026-09-15 重建四仓镜像并上线，烤入 gq `94c27be` / cs `16f92c0` / sp `4fbd631` / cc `68dbff1`，**抽样指纹 7 点 MATCH**；**残余 = 行为面**，见 §4 第 5 条） |
...
| **批 6** | P4-1 | **镜像重建后复验** | ✅ **已完成（2026-09-15，批 6a）** —— 四仓镜像重建上线 + 7 抽样点指纹 MATCH，记录在 online `task.md` T-3.17（该条 2026-09-16 结清）。**⚠️ 行为面残余未验收**，见 §4 第 5 条 |
...
> **本节现状（2026-09-16）**：原 6 条，**可验项已全部结清 4 条**（第 1 · 2 · 3 · 5 条），
> **余 2 条「边界条目」均标 `【不可关闭】`**（第 4 · 6 条），**不再排期**。
> 判据见 §7 约定 4：可验项验完即删；边界条目**须带失效条件**、标注后保留。
> ⇒ **看到「未验清单」不要默认还有可验项** —— 本节的可验面已清零。
...
2. ~~**P1-1 只读了两处**：客户端 `backflow_client.py` 与 `error_push.py`；**其余调用方是否另有分流未读**。~~
   ...
   **无第三个调用方**。分流面逐一核过：`error_push.py:171` 的 `if not _is_retryable(exc)`
   是**唯一**分流点；`pull_loop.py:261` / `:344` 两处 `except backflow_client.BackflowClientError:`
   **均不看 `status_code`**（另 `pull_loop.py:88` 有一处裸 `except Exception`）⇒ 与 §5.3 的定论
...
4. **【不可关闭】** **`/perf`、`/cost`、`/compare` 判「无对象」是取证结论，不是「验证过没问题」**。
   **失效条件**：一旦 error run 开始写 perf/cost（即 error 通道的 `timing`/`usage` 出现可聚合的真实值），
   **这三面要回头重判** —— 在此之前该结论恒成立、无从证伪（自证一次不够，它描述的是**未来时**）。
   ⚠️ 与本条相邻的**可验**事实已单列并结清：桩执行产物 `usage=[null]`/`timing=[{}]` 见第 3 条。
5. ~~**P4-1 的四条真机项**本轮**未复验**，状态取自台账自陈（`task.md:299/:300`）。~~
   ✅ **已订正（2026-09-16）—— 镜像面已复验，本行原述**已过期**（原写的是「本轮未复验」，而**批 6a 早在 2026-09-15 就做了**）**：
   四仓 backend 镜像**已重建并上线**，烤入提交 = gq `94c27be` / cs `16f92c0` / sp `4fbd631` / cc `68dbff1`，
   **抽样指纹 7 点 sha256（两侧 `tr -d '\r'` 归一）全部 MATCH（3/1/2/1）**；平台两仓（online/offline）
   同日第二阶段一并重建。**T-3.17 的待裁 ②③ 已于 2026-09-16 裁定并结清**（见 online `task.md` T-3.17「本条结清」）。
   **⚠️ 残余（属行为面，非镜像面，不得被上面这段盖过）**：① **T-3.18 的「取消是否真落在那两个窗口」仍未观测**
   ——本批只证「落了就不丢账」，**不证「一定会落」**；② **`error_type` 折叠后的行为只有 cs 有真机样本**，
   gq（唯一那条 error 在重建前）/ sp（13 条 `llm_call` 全 ok）/ cc（零样本）**仍无**（`task.md:314`）。
   ⚠️ **判据面不许读大**：指纹复验是**7 个抽样点的正面比对，不是逐文件全比** ⇒「MATCH」只证明**这些点**
   与宿主 HEAD 逐字节一致，**不证明镜像内其余文件无残留旧码**。
6. **【不可关闭】【2026-09-16 新增】⑥环的「补判」路径真机零案例**：`api/backflow.py:1087` 的 `cluster.status=='claim'` 守卫会把判定**静默跳过**，之后靠「后续推送」或 `worker/rejudge_job.py` 补判。**已查全库（`dev.obs`，2026-09-16）：`auto_fixed=6` 那 6 条 `passed` link 的 cluster 全程在 `claim` 态被判，无一条走过「先被守卫跳过、事后补判」这条路** ⇒ 该路径**只有代码存在性证据，无真机案例**。不许据 `rejudge_job` 文件存在就写「兜底已验证」。
   ...
   **失效条件**：真实运维中一旦出现「推送到达时 cluster 非 `claim`」的案例，即可**顺带取证**（无需造数据）；
   或排期单独做**带基线标注的受控实验**（造前记基线水位、造完当场把「这批红是我造的 + 起止时刻 + 撤销动作」
   写进本仓台账，撤销后回读进程内的值 —— 见 online `task.md` 同族教训）。**在此之前不得据 `rejudge_job` 文件存在
   就写「兜底已验证」。**
...
- ~~**不证明 requeue 之外的恢复路径可用**：`manual_invalidate` 竞态对账（§5.9）本批**未真机触发**。~~
  ...
  **如实记载在两处**：`error-backflow-status.md` O-D.4（「二、部分落地」，该行本就写着「非 200 一律抛，
  ...
  ⇒ **本项由「未验收项」改判为「未实现项」**。
  **➜ 收口（2026-09-16，评审裁定选 A）= 不补**：影响面评估（两侧库只读直查）得**触发条件实测零次**
...
| ~~C-3~~ ✅ | ~~**lint 门禁缺口**：ruff 本机不可用（三处 venv 均无）；**offline 仓无任何 CI** ⇒ 唯一门禁是**手动**借 online venv + online 配置扫改动文件~~ ⇒ **✅ 已结清（2026-09-16，#321）**，**本行原述已过期两处**：① **ruff 可跑**（借 online 仓 venv 的 `ruff.exe` 0.16.6，宿主 PATH 确无但可借）；② **判据已随仓走** —— 本仓新建 `backend/ruff.toml`（`py311` / `line-length 100` / `select=[E,W,F,I]` + `known-first-party=["app"]`，对齐 online 口径；**只声明 lint、不声明依赖**，依赖仍以 `requirements.txt`/`lock` 为准）⇒ 「借 online 配置」这一说法**不再成立**。复核命令与存量基线写在本仓 `backend/ruff.toml` 头部与该次提交。**仍未做**：offline **仍无任何 CI**（判过不做，属「文档承诺 ≠ 需求」族）；且「零新增」口径**永久合法化存量 594 条**（含 `F821`×12 / `B008`×116 等真信号）——**这是刻意接受的口径，不是干净** | 长期未验收项 |
...
4. **§4 未验清单**：**可验项验完即删**（验一个删一条）；**不可关闭的「边界条目」标注后保留**，不许删。
   **新增未验项必须同时进 §4**，不许只写在 §1。
   **为什么改这句**（原句是「只能缩短」，2026-09-16 修订）：原句把 §4 读作**待办清单**，
   隐含「所有条目最终都能被验掉」。但 §4 里存在一类**性质上永远验不掉**的条目 ——
   **未来时限定语**（「X 一开始发生就要回头重判」）与**已裁定不做**（取证代价 > 收益，如造数据注入共享观测面）。
   它们**留着才是有用信息**，删掉等于把边界抹掉。原句会让每个后来人都在这类条目上白费一轮（本轮实测）。
   **边界条目的书写要求**：① 标 `【不可关闭】`；② **必须写失效条件**（「什么情况一变，这条就该重新判」），
   没有失效条件的边界条目等于死状态；③ 与可验项在视觉上区分开。
