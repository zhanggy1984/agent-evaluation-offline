...
class TestOverfitOnGateCards(unittest.TestCase):
    """6.4b L0 门禁降级：overfit_gap = 最近 manual − 最近 held_out，超阈值标 overfit。"""

    def test_no_held_run_no_overfit(self):
        """无 held_out run（或 held 无分）→ gap None、overfit False（数据不足不误报）。"""
        manual = _run(1, 1, 10, "1.0.0", "completed", _dt(2), agent_score=80.0)
        cards = build_gate_cards([_agent(1)], [manual], [_suite(10, 1, "s1")], {10: 1})
        self.assertIsNone(cards[0]["overfit_gap"])
        self.assertFalse(cards[0]["overfit"])

    def test_gap_computed_below_threshold(self):
        manual = _run(1, 1, 10, "1.0.0", "completed", _dt(1), agent_score=80.0)
        held = _run(2, 1, 10, "1.0.0", "completed", _dt(2), agent_score=70.0,
                    trigger_type="held_out")
        cards = build_gate_cards([_agent(1)], [manual, held], [_suite(10, 1, "s1")], {10: 1})
        self.assertEqual(cards[0]["overfit_gap"], 10.0)
        self.assertFalse(cards[0]["overfit"])

    def test_overfit_above_threshold(self):
        manual = _run(1, 1, 10, "1.0.0", "completed", _dt(1), agent_score=90.0)
        held = _run(2, 1, 10, "1.0.0", "completed", _dt(2), agent_score=60.0,
                    trigger_type="held_out")
        cards = build_gate_cards([_agent(1)], [manual, held], [_suite(10, 1, "s1")], {10: 1})
        self.assertEqual(cards[0]["overfit_gap"], 30.0)
        self.assertTrue(cards[0]["overfit"])

    def test_threshold_param(self):
        """阈值由调用方传入（全局热配置读入）；threshold=5 → gap10 超阈值。"""
        manual = _run(1, 1, 10, "1.0.0", "completed", _dt(1), agent_score=80.0)
        held = _run(2, 1, 10, "1.0.0", "completed", _dt(2), agent_score=70.0,
                    trigger_type="held_out")
        cards = build_gate_cards([_agent(1)], [manual, held], [_suite(10, 1, "s1")], {10: 1},
    ...
    def test_recent_runs_win(self):
        """取最近 manual 与最近 held_out（按 started_at），历史 run 不干扰。"""
        old_manual = _run(1, 1, 10, "1.0.0", "completed", _dt(1), agent_score=50.0)
        new_manual = _run(2, 1, 10, "1.1.0", "completed", _dt(3), agent_score=90.0)
        held = _run(3, 1, 10, "1.1.0", "completed", _dt(2), agent_score=60.0,
                    trigger_type="held_out")
        cards = build_gate_cards([_agent(1)], [old_manual, new_manual, held],
                                 [_suite(10, 1, "s1")], {10: 1})
        self.assertEqual(cards[0]["overfit_gap"], 30.0)  # 90 - 60

    def test_held_without_score_ignored(self):
        """held_out run 无评分（timeout 等）不参与 overfit 判定。"""
        manual = _run(1, 1, 10, "1.0.0", "completed", _dt(1), agent_score=80.0)
        held = _run(2, 1, 10, "1.0.0", "timeout", _dt(2), trigger_type="held_out")
        cards = build_gate_cards([_agent(1)], [manual, held], [_suite(10, 1, "s1")], {10: 1})
        self.assertIsNone(cards[0]["overfit_gap"])
        self.assertFalse(cards[0]["overfit"])

    def test_held_avg_over_window(self):
        """留出集表现 = 最近 N 次 held_out 均值：90 − (60+90+90)/3 = 10，单次低分不误报。"""
        manual = _run(1, 1, 10, "1.0.0", "completed", _dt(1), agent_score=90.0)
        held = [
            _run(2, 1, 10, "1.0.0", "completed", _dt(2), agent_score=60.0, trigger_type="held_out"),
            _run(3, 1, 10, "1.0.0", "completed", _dt(3), agent_score=90.0, trigger_type="held_out"),
            _run(4, 1, 10, "1.0.0", "completed", _dt(4), agent_score=90.0, trigger_type="held_out"),
        ]
        cards = build_gate_cards([_agent(1)], [manual] + held, [_suite(10, 1, "s1")], {10: 1})
        self.assertEqual(cards[0]["overfit_gap"], 10.0)
        self.assertFalse(cards[0]["overfit"])
    ...
    def test_window_param(self):
        """窗口 N 由调用方传入（全局热配置读入）；window=1 → 只取最近 held_out。"""
        manual = _run(1, 1, 10, "1.0.0", "completed", _dt(1), agent_score=90.0)
        held = [
            _run(2, 1, 10, "1.0.0", "completed", _dt(2), agent_score=60.0, trigger_type="held_out"),
            _run(3, 1, 10, "1.0.0", "completed", _dt(3), agent_score=90.0, trigger_type="held_out"),
        ...
        cards = build_gate_cards([_agent(1)], [manual] + held, [_suite(10, 1, "s1")], {10: 1},
                                 overfit_window=1)
        # 窗口 1 → 只取最近 held_out（90）→ gap=0；若窗口 3 均值 75 → gap=15 会误报
        self.assertEqual(cards[0]["overfit_gap"], 0.0)
        self.assertFalse(cards[0]["overfit"])
