...
⚠️ 两条路径的 `total_case` 不同源，故「未完成 case 回填 na」的断言只在硬超时路径成立：
`total_case` 只在 pending→running 的接管 UPDATE 里被赋值（orchestrator），
**建单处不写**（`_create_error_regression_run_locked` 未传该列）⇒ 从未被接管的 pending
error run 的 `total_case` 恒为模型默认 0，`_finish_error_regression` 的
`len(results) < run.total_case` 判据为假、不触发对账回填。这是**先于本批存在的独立缺口**，
不属本批范围（见 status 表登记），故此处**不**对它断言。
"""
import pytest
...
from helpers import create_chain, make_run  # noqa: E402
...
# ---------------- ②租约路径（pending 未被接管 ⇒ total_case 仍为 0） ----------------

@pytest.mark.asyncio(loop_scope="session")
async def test_reap_error_run_pending_lease_short_circuits_salvage(db, env, monkeypatch):
    """租约回收 pending error run（orchestrator 未接管）：**不产分、不落 'error' 行**。

    ⚠️ 本用例不设 `total_case` —— 建单处本就不写它，这才是该路径的真实现场。
    """
    ch = await _seed_error_chain(env, db, ["it256-pd-a", "it256-pd-b"])
    now = _utcnow()
    run = _make_error_run(ch)                    # total_case 保持模型默认 0
    run.status = "pending"
    ...
    ers = await _results_of(run.id)
    assert [e.pass_fail for e in ers] == ["pass"], "已采集行被重算 ⇒ 走了 salvage（未短路）"
