...
只测 candidates_to_purge（保留 N 次 / 排除 pinned / 排除 issue 关联 / 排序）；
...
class TestCandidatesToPurge(unittest.TestCase):
    def test_retain_keeps_newest_per_group(self):
        ...
        self.assertEqual(candidates_to_purge(runs, set(), 2), [1])
    ...
    def test_grouped_by_agent_and_suite(self):
        ...
        self.assertEqual(candidates_to_purge(runs, set(), 1), [1, 4])
    ...
    def test_pinned_excluded(self):
        ...
        self.assertEqual(candidates_to_purge(runs, set(), 2), [])
        # pinned 排除后剩 2 个，retain=1 → 只删最老的非 pinned（run2）
        self.assertEqual(candidates_to_purge(runs, set(), 1), [2])

    def test_issue_referenced_excluded(self):
        """被 issue 引用的 run 不清理（保留证据链）。"""
        runs = [_run(1, 1, 10, _dt(1)), _run(2, 1, 10, _dt(2)), _run(3, 1, 10, _dt(3))]
        # issue 引用 run2 → 排除；retain=1 → 删 run1
        self.assertEqual(candidates_to_purge(runs, {2}, 1), [1])

    def test_below_retain_no_candidates(self):
        self.assertEqual(candidates_to_purge(
            [_run(1, 1, 10, _dt(1)), _run(2, 1, 10, _dt(2))], set(), 5), [])

    def test_oldest_first_sorted(self):
        """候选按 id 升序（分批删除优先清最老数据）。"""
        runs = [_run(9, 1, 10, _dt(9)), _run(3, 1, 10, _dt(3)), _run(7, 1, 10, _dt(7))]
        self.assertEqual(candidates_to_purge(runs, set(), 0), [3, 7, 9])

    def test_empty_input(self):
        self.assertEqual(candidates_to_purge([], set(), 50), [])
    ...
    def test_version_representative_protected(self):
        ...
        self.assertEqual(candidates_to_purge(runs, set(), 2), [1, 2, 4])
