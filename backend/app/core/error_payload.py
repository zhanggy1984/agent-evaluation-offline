...
对 `keyword_contains` 是假绿方向。**本批未加长度闸，但这不等于「量化后确认无需加」**——
真库实测该面实例数为 0，而那个 0 取自一个**结构上装不下此风险的样本**：词表侧的唯一输入
是回流信封，而 `error_backflow_inbox` 现为 **0 行**，该面从未被填过。故实为**量不出**，
只能按不加闸处理。待批 B/C 出现真实回流词表后**重估**（那时的词表来自 online 兜底话术配置，
属自然语言，`AI`/`API` 这类短 ASCII token 才真正可能出现）。
"""
