...
- 净化/命中规则须与执行端 `KeywordNotContainsOp` 子串匹配语义对齐（**大小写敏感核对并归一**，批 2 §7.2 命中一致性 / 安全 4），防「边界词致恒 fail」或「大小写不一致致漏判恒 pass」。
  ...
  - ⚠️ **本次量化 ≠ 折叠安全**：真库实测 94 条 keyword 断言 / 934 个「断言×结果」组合，判定翻转 **0** 条——但可折叠面（含 ASCII 大写的关键词）**只有 2 个**（`DSML`/`Docker`），0 的成因是**样本薄**而非无风险。短 ASCII 关键词误命中面（`"AI"` 命中 `"he said"`）实例数 **0**，故**未加长度闸**；该代价仍为开放项，见 `core/error_payload.py` 文件头。
