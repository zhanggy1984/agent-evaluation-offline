...
> **定位**：批 1 + 批 2 的**编码级详设**（= `backend/` 平台仓改造的实施规格）。**叠加基线**：批 1 = `error-backflow-phase1.md` **v0.3.0**；批 2 = `error-backflow-phase2.md` **v0.8.0**；消费契约 = online `solution_detail.md` **v1.23** / `solution.md` **v3.5.9** / `task.md` 环 0（R-13~R-24 实现清单登记，L49）。
>
...
- 净化/命中规则须与执行端 `KeywordNotContainsOp` 子串匹配语义对齐（**大小写敏感核对并归一**，批 2 §7.2 命中一致性 / 安全 4），防「边界词致恒 fail」或「大小写不一致致漏判恒 pass」。
  - ✅ **2026-09-14 结清（#235）**：核对结果 = 执行端原为**大小写敏感**（`k in val`），而净化侧归一小写 ⇒ 正是本条预言的「大小写不一致致漏判恒 pass」（词表折成 `dsml`，`"<DSML>"` 匹配不上，金丝雀静默放行）。两侧已同批改为大小写不敏感：算子侧共用 `_keyword_hits`（结构性约束，见 `assertions/ops/text.py` 模块头），词表侧 `sanitize_words` 归一小写。
  - ⚠️ **本次量化 ≠ 折叠安全**：真库实测 94 条 keyword 断言 / 934 个「断言×结果」组合，判定翻转 **0** 条——但可折叠面（含 ASCII 大写的关键词）**只有 2 个**（`DSML`/`Docker`），0 的成因是**样本薄**而非无风险。短 ASCII 关键词误命中面（`"AI"` 命中 `"he said"`）实例数 **0**，**未加长度闸**；但该 0 **不等于「确认无需加闸」**——词表侧输入只来自回流信封，而 `error_backflow_inbox` 现为 0 行，该面从未被填过 ⇒ 实为**量不出**，按不加闸处理，待批 B/C 有真实词表后**重估**。该代价仍为开放项，见 `core/error_payload.py` 文件头。
...
- **口径澄清（v1.23 补）**：上述载荷的 `trigger_signal_id` = **该 run 归属的 online cluster id**（取值 = `backflow_envelope['source']['cluster_id']`），**非** offline `eval_run.trigger_signal_id`（§6.1 consumed 锚）—— 同名非同物；填错**静默不报错**（撞上 pending link → 推进错簇；撞不上 → orphan 且 online 仍回 200 → 结果永久丢失），详见 phase2 §9.3 v1 硬约束注；**⚠️ 与「填错」相反，「缺失/null」不静默**——v1.23 幂等键改造（online 键 `(link_id, run_id)` → `(cluster_id, run_id)`）后该字段被收为**必填**，缺失/null **online 侧 422 拒单**（可感知）。两种失败模式必须分开记：填错 = 200 + 结果走错簇/丢失，缺失 = 422 + 本笔全丢。**本发送侧不受影响**：`error_push._cases_of_cluster` 按信封簇 id 分片组装，**取不到簇 id 的 case 根本不进载荷**（就地记 error 日志丢弃），故不会推出 null；**接收方权威口径 = online `solution_detail.md` §8.7 载荷字段表**。
