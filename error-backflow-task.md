...
- **O-D.5 fake-online stub 基建（环 1，`backend/tests/`）**：fixture 回放 online pull/ack 响应 + **入站鉴权（pull_token）形状**（出站静态预共享 secret 形状归环 2 O-F.7/O-F.8）（R1~R3 + R-4~R-24 修订契约 shape），供 O-D.2~O-D.4 驱动（详设 §12.5 验证层级 + 批 1 §12.2 环 1：fixture + fake-online，无真实 online，C3 只入测试基建）。**验证目标**：stub 驱动收单/建 case/ack 全链路仓内全绿。
  ...
  - **✅ 2026-09-15 真机施行记录（6b-2/3，④⑤⑥ 环）**：**无代码改动**。信号 run = eval_run **3660**
    ...
    **2026-09-15 同日实测补充（加强，不改定性）**：真实形状已确证 = good-question 的
    ...
    `input_type` **硬编码 `"text"`**、**未走**「与 agent 模板域自洽」的校验。（⚠️ 2026-09-15 订正：原文写「未走接口字段推导」，
**错** —— `core/skeleton.py:53-55` 的 `_input_type` 本就**不按接口推导**，而是按数据自身的
...
> 再按真实载荷倒推该写什么断言**。恢复时机 = 第一条数据打通后由评审裁定。
...
| R-8 | cap_gap 探测态节流（invalidated ack 只发一次 + 本地探测补齐） | offline ack 语义 | O-D.4 | G3 | 探测节流断言；§12.1 R-8 行 · **✅ 探测态自愈扫描已落码（2026-09-16）**：`runner/cap_gap_probe.py`（`CAP_GAP_PROBE` 首次落码 + `probe_once()` + 低频 loop，自检与拉取路径同源 `pull_loop._self_check`）+ `main.py` 注册（含 shutdown cancel）；单测 8 条、全量回归 990/100、ruff 零新增、真库探针 10/10。**⚠️ 真机只覆盖「仍缺 → 静默」一支**，「补齐 → 建 case + ack active」与 online 侧 R2 例外**无真机证据**（理由见 `error-backflow-pending-phases.md` §5.9 ⑤）|
...
| **R-28** | **`online_content_gap` 拒绝行的恢复路径被自家重处理谓词挡死**：详设 §5.9 / O-D.4 给 `online_content_gap` 的恢复路径写作「**等 requeue 重拉覆盖**」，但 online 侧 `requeue_link` **复用同一 `payload_id`** 重组装 ⇒ 该行落回离线走的是**就地重处理**、不是「新拉取」；而重处理复位谓词又限 `ack_status ∈ {none, pending}`，被拒行落地时 `ack_status` 已 `acked` ⇒ **词表补齐后重新 requeue 的载荷被自家谓词挡在门外、只能反复丢弃**。根因 = `reject_code` 在离线仓**无任何读取者**（全仓 grep 零命中），谓词从不看它 ⇒ 「按 `reject_code` 分流」这一层**从未落码**。 | offline runner（**A 级**：改重处理谓词） | O-D.4（§5.9） | — | **✅ 已修（2026-09-16，`bba5625`）**：`runner/pull_loop.py` 抽出 `_needs_reprocess` 按 `reject_code` 分流 —— `online_content_gap` **一律重处理**（不限 ack_status）、`offline_cap_gap` 仍限 `{none, pending}`（详设 §5.8 的 R-8 探测态节流语义**不变**）；`_inbox_put` 按 `payload_id` upsert，避免重处理时重复插行。单测 `test_pull_loop_requeue.py`（12 条）+ `test_pull_loop_reject.py`（23 条）、全量回归 982 passed / 100 skipped；**真机复验**见 `error-backflow-pending-phases.md` §5.8。**同族登记（当时不实现）**：详设 §5.8 定义的 `CAP_GAP_PROBE` 常量在离线仓**同样零实现落点**（R-8 探测态节流未落码）—— 当时仅登记，**已于 2026-09-16 单独落码验收**（见本表 R-8 行 + `error-backflow-pending-phases.md` §5.9；自愈支无真机证据）|
...
- **回流链在当前环境下从未真跑（2026-09-15 实测；6b 行为面验收的前置条件）**：`docker-compose.yml:52` 写 `BACKFLOW_ENABLED: ${BACKFLOW_ENABLED:-false}`，而**根 `.env` 无该键**（逐名 grep 零命中，仅 `.env.example:51` 有）⇒ 容器内恒为 `false`。实测启动日志：`回流拉取未启用（BACKFLOW_ENABLED=false），跳过启动`（`runner/pull_loop.py:73`）+ `差集对账未启用（BACKFLOW_ENABLED=false），跳过启动`（`runner/reconcile_loop.py:141`）。**该默认是设计如此**（`phase1.md:513`：「功能开关（默认 off；联调/上线打开）」），**不是缺陷**。**但推论必须记牢**：本 task 的 pull / reconcile / 判定主干**至今没有一次「由运行中 loop 产生」的真机证据** —— 既往所有真机验收都是**探针直接跑代码**得出的（探针不经 `main.py` 门控、不改开关），二者**不是同一个证据面**。⇒ **凡以「运行实例行为」为证据面的验收（6b 行为面即属此类），前置条件是先开开关**，且开关一开 `_validate_secrets` 会强制两 secret 非空 ≥256bit + `BACKFLOW_ONLINE_API_BASE` 为合法 URL，缺一即拒绝启动（`phase1.md:532`）。**开开关 = 改配置（A 级），须先出方案，不得顺手打开。**
